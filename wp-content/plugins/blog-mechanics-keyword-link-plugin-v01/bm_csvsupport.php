   ...
   function bm_keywordlink_cvsexport()
	{
		...
		echo "Keyword,URL,NoFollow,First Only,New Window,Ignore Case,IsAffiliate,Enable In Comments\n";
	 	...
	 	foreach ($links as $keyword => $details)
		{
			   list($link,$nofollow,$firstonly,$newwindow,$ignorecase,$isaffiliate,$docomments) = explode("|",$details);
				...
				bm_keywordlink_cvsexportvalue($docomments,true);
	...
	function bm_keywordlink_cvsimport()
	{
		...
		if (is_uploaded_file($_FILES['upload']['tmp_name'])) 
		{
			...
			foreach($cvscontent as $row => $value)
			{
				...
				list($keyword,$link,$nofollow,$firstonly,$newwindow,$ignorecase,$isaffiliate,$docomments) = explode(",",$value);
				...
				if ($docomments) $docomments = 1;
	  			...
	  			$newlinks[$keyword] = implode('|',array($link,$nofollow,$firstonly,$newwindow,$ignorecase,$isaffiliate,$docomments));
			}
