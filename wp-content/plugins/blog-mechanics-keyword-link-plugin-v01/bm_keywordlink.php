...
function bm_keywordlink_showdefinitions()
{
		 ...
		 { 
 	     ...
 	     echo "<thead><tr><th>#</th><th>Keyword</th><th>Link</th><th>Attributes</th><th>Action</th></tr></thead>\n";
			 $cnt = 0;
		 	 ...
		 	 {
				list($link,$nofollow,$firstonly,$newwindow,$ignorecase,$isaffiliate,$docomments) = explode('|',$details);
			   ...
			   if ($cnt++ % 2) echo '<tr class=alternate>'; else echo '<tr>';
				...
				$urlsave_keyword = base64_encode($keyword); 	
				$urlsave_url 	  = base64_encode($link);
				
				echo "<td>";
				echo "<input type=button value=Edit onClick=\"javascript:BMEditKeyword('$urlsave_keyword','$urlsave_url',";
				echo (($nofollow=="0")?"0":"1") . "," . (($firstonly=="0")?"0":"1") . "," .(($newwindow=="0")?"0":"1"). "," .(($ignorecase=="0")?"0":"1") . "," . (($isaffiliate=="0")?"0":"1") . "," . (($docomments=="0")?"0":"1") . ");\"/>";
		...
		function BMEditKeyword(keyword,url,nofollow,firstonly,newwindow,ignorecase,isaffiliate,docomments)
		{
			 document.bm_keywordadd.keyword.value      = decode64(keyword);
			 document.bm_keywordadd.link.value         = decode64(url);		
...
function bm_keywordlink_savenew()
{
      $links = get_option(BM_KEYWORDLINK_OPTION);
		...
		if (isset($links[$keyword]))
		{
 		...
 		/* Store the link */ 
	  $links[$keyword] = implode('|',array($link,$nofollow,$firstonly,$newwindow,$ignorecase,$isaffiliate,$docomments));
